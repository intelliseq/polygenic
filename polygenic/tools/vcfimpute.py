...
def impute(index: int, ld_threshold: float, ref_row_array: list, target_row_array: list):
    ...
    for ldproxy_index in ldproxy_order:
        ...
        print(ldproxy)
        print(ref_row_array[index][:30])
        print(target_row_array[ldproxy_order[0]])
        print(target_row_array[ldproxy_order[1]])
        print(target_row_array[ldproxy_order[2]])
        print(target_row_array[ldproxy_order[3]])
...
def run(args):
    ...
    for record in ref_vcf:
        ...
        if target_row_array[middle_index] == [None, None]:
            ...
            print(imputed)
            break
        #print(target_haplotypes[1][args.window - 1])
        if counter < args.window:
            continue
        if counter == args.window:
            pass
            #print(array_pos[0])
            #print(target_haplotypes[0][0])
        if counter >= args.window:
            pass
            


    # print(target_haplotypes)
    # print(target_dict)
